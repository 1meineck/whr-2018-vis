function RadarChart() {
    ...
    chart.addData = function (array) {
        ...
        if (!alreadyThere && data.length < options.maxLength) {
            new_set = {
                "key": array.Country,
                "values": [
    ...
    chart.replaceData = function(array){
        data.pop()
        data.add(array)
    }

    // Removes Data with a specified key from the dataset
    // should be used in LegendClick only at this stage
    ...
    chart.maxLength = function(value){
        if (!arguments.length) return options.maxLength;
        options.maxLength = value;
        console.log(options.maxLength)
        return chart;
    };
    
    chart.stacking = function (value){
        if (!arguments.length) return options.stacking;
        options.stacking = value;
        console.log(options.stacking)
        return chart;
    };
